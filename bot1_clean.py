...
def calc_heuristic(state: Board):
    ...
    # --- Add a small penalty for non-pawn pieces that haven't left their starting squares ---
    # Define the starting squares (using 0-63 indexing) for non-pawn pieces.
    white_starting = {
        KNIGHT: {1, 6},
        BISHOP: {2, 5},
        ROOK: {0, 7},
    ...
    # Set the penalty value (adjust this value as needed)
    undeveloped_penalty = 0.1

    # For White: subtract the penalty for every piece that hasn't left its starting square.
    for piece_type, starting_squares in white_starting.items():
        for square in state.pieces(piece_type, WHITE):
            if square in starting_squares:
                value -= undeveloped_penalty

    # For Black: add the penalty (since it's a drawback for Black, which is a plus for White).
    for piece_type, starting_squares in black_starting.items():
        for square in state.pieces(piece_type, BLACK):
            if square in starting_squares:
                value += undeveloped_penalty
